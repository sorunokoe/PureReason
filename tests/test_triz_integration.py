        ...
        try:
            ...
            if flags_risk == scenario["expected_risk"]:
                ...
                if 0.80 < similarity < 0.92:
                    print(f"  ⚠️  BORDERLINE: Near threshold, acceptable")
                    passed += 1
                else:
                    print(f"  ❌ FAILED")
                    failed += 1
...
def test_meta_learner():
    ...
    try:
        ...
        if result.returncode == 0 and "test result: ok" in result.stdout:
            ...
            print(f"❌ Meta-learner tests failed")
...
def test_domain_calibration():
    ...
    try:
        result = subprocess.run(
            ["cargo", "test", "--package", "pure-reason-core", "--lib", "domain_calibration::tests"],
            capture_output=True,
            text=True,
            timeout=60,
            cwd=Path(__file__).parent.parent
        ...
        if result.returncode == 0 and "test result: ok" in result.stdout:
            ...
            print(f"❌ Domain calibration tests failed")
            print(result.stdout[-500:])
            return 0, 1
