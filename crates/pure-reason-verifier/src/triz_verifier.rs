...
use pure_reason_core::{
    pre_verification_v2::{PreVerifier, PreVerificationConfig},
    meta_learner_v2::{SessionMetaLearner, MetaLearnerConfig, EnsembleWeights},
    semantic_fallback::{SemanticFallbackDetector, SemanticFallbackConfig},
    domain_calibration::{DomainCalibrator, DetectedDomain},
...
impl TrizVerifierService {
    ...
    pub fn verify(&self, req: super::VerificationRequest) -> Result<super::VerificationResult, super::VerifierError> {
        ...
        let domain_weights = detected_domain.as_ref().map(|d| d.config.ensemble_weights.clone());
        
        // Merge weights: domain > adaptive > default
        let _final_weights = domain_weights
            .or(adaptive_weights)
            .unwrap_or_default();
        ...
        if let Some(ref domain) = detected_domain {
            ...
            if let Some(metadata) = result.metadata.as_object_mut() {
                metadata.insert("domain".to_string(), serde_json::json!({
